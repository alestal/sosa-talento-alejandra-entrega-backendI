...
exports.createProduct = async (req, res) => {
  ...
  if (!id || !title || !description || !code || !price || status === null || stock === null || typeof category !== 'string' || !Array.isArray(thumbnails)) {
    return res.status(400).json({ error: "Faltan datos obligatorios" });
  }

  const newProduct = await managerProduct.addProduct({ id, title, description, code, price, status, stock, category, thumbnails });
